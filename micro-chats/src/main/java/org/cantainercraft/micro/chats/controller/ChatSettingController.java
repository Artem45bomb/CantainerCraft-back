...
public class ChatSettingController
{
    ...
    @PreAuthorize("hasAnyRole('USER,ADMIN')") //<<<--------------------
    @PostMapping("/chat")
    public ResponseEntity<List<Chat_Settings>> findByChatId(@RequestBody UUID uuid) {
        List<Chat_Settings> chatSettings = chatSettingsServiceImpl.findByChatId(uuid);
        if(chatSettings.isEmpty()) {
            throw new NotResourceException("NoN");
        }
        return ResponseEntity.ok(chatSettings);
    }
